...
\end{enumerate}
For the bottom drag input, drag.gr3 uses drag coefficient (\hl{=bottom roughness in meters? @Joseph}) and manning.gr3 uses the Manning's number $n$. Rough.gr3 uses $C_D$ bottom roughness in meters with specific equations. \\
...
\begin{figure}
    ...
    \label{fig:hgridman}
\end{figure}
You can also use BlueKenue to create a spatially varying Manning.gr3. See the steps in the pdf under /src/Utility/Pre-Processing/step\_by\_step\_bottom\_friction\_BK.pdf.
\subsubsection{Rough.gr3}
\hl{not clear, @Joseph could you check and explain more please?} \\
Bottom roughness in meters are read from rough.gr3. In this case, negative or 0 depths in rough.gr3 indicate time-independent $C_D$, not roughness. With this option, two additional parameters are needed in param.nml: $dzb_{min}$ in meters and $dzb_{decay}$ nondimensional. Information can be found in the section 4.2.4.2 of the manual and in param.nml for the calculation of $C_D$ for rough.gr3:
\begin{lstlisting}
...
\end{lstlisting}
Put the file created in a folder "sflux" with a file "sflux\_inputs.txt" \hl{@Joseph: why we need this file}, and this folder in the folder of your run (Figure \ref{fig:sflux}).\begin{figure}[hbtp]
    \centering
...
\noindent For IBOUNDFORMAT=2: write the name of the 5 nc files in a file called 'bndfiles.dat', \hl{the order is very important, to be confirmed someone???}: \begin{enumerate}
    \item dir
    \item fp
    \item hs
    \item spr
...
For IBOUNDFORMAT=3, you have to specify the names of binary files \hl{@Xavier: could you add information here please?}
%%%%%%%%%%%%%%%%%%%%%%%%
...
\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\acknowledgments{\hl{Thanks to Xavier Bertin, Joseph Zhang, Vanessa Hernaman, Jerome Lefevre, Ivica Janekovic, Jaime Calzada and Roger Proctor.} }
...
\appendix
\section{}
\hl{These scripts will be put in Github "in progress"}
\unskip
...
nope2 = int(bnd_in[0][0])
ifl_wwm =[]
#for i in range(nope2):
for i in range(len(bnd_in)-1):
    x = bnd_in[i+1].split(' ')
    ifl_wwm.append(int(x[1]))

print(ifl_wwm)

with open('hgrid_last.gr3') as f:
    ...
    nolb= int(nolbt[0])
    neta2=next(fr)
    for k in range(nolb): #nolb
        rect= next(fr)
        nlnd= int(rect[0]) #indicate number of nodes for land/island
        #nlndf=int(rect[1]) #indicate 0 for land and 1 for island
        print(nlnd)
        #print(nlndf) not working....
        for i in range(nlnd):
            rec=next(fr)
            ilnd=int(rect[0])
            ilndf=int(rect[1]) #indicate  0 for land and 1 for island????
            print(ilndf)
            if i == 0: print(rect,ilnd)
            if ilnd>np or ilnd<=0:
                print('ilnd>np')
                exit()
            #ibnd[ilnd-1]=ifl_wwm[2]
            if ilndf == 0: 
             ibnd[ilnd-1]=ifl_wwm[2]
            elif ilndf == 1:
             ibnd[ilnd-1]=ifl_wwm[3]
         
#     Output
#     Output file
with open('wwmbndtest.gr3','w') as w:
